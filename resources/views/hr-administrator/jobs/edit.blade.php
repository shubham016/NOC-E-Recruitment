...
@section('user-name', Auth::guard('hr_administrator')->user()?->name ?? 'Guest')
@section('user-role', 'HR Administrator')
@section('user-initial', Auth::guard('hr_administrator')->user() ? strtoupper(substr(Auth::guard('hr_administrator')->user()->name, 0, 1)) : 'H')
...
@section('sidebar-menu')
    ...
    <a href="{{ route('hr-administrator.jobs.index') }}" class="sidebar-menu-item active">
...
@section('custom-styles')
    <style>
        .page-header {
            ...
            margin-bottom: 2rem; box-shadow: 0 4px 12px rgba(220,38,38,0.3);
        }
        .govt-badge {
        ...
        .form-control:focus, .form-select:focus { border-color: #dc2626; box-shadow: 0 0 0 0.2rem rgba(220,38,38,0.15); outline: none; }
        ...
        .form-check-input:focus { border-color: #dc2626; box-shadow: 0 0 0 0.2rem rgba(220,38,38,0.15); }
        .inclusive-subcategory { max-height: 0; overflow: hidden; transition: max-height 0.3s ease, margin 0.3s ease, opacity 0.3s ease; opacity: 0; }
...
@section('content')
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-start">
            ...
            <a href="{{ route('hr-administrator.jobs.index') }}" class="btn btn-light btn-lg">
    ...
    <div class="info-alert">
        <div class="d-flex align-items-start gap-3">
            ...
            <div>
                <strong>Editing Vacancy:</strong> Advertisement No. <span class="fw-bold text-primary">{{ $job->advertisement_no }}</span>
    ...
    <form method="POST" action="{{ route('hr-administrator.jobs.update', $job->id) }}" id="vacancyForm">
        @csrf
        @method('PUT')
                ...
                <div class="form-card">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                            ...
                            value="{{ old('advertisement_no', $job->advertisement_no) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select form-select-lg @error('position_level') is-invalid @enderror"
                            ...
                            <optgroup label="Officer Level (अधिकृत तह)">
                                <option value="Officer Level - 10th (अधिकृत तह - १०)" {{ old('position_level', $job->position_level) == 'Officer Level - 10th (अधिकृत तह - १०)' ? 'selected' : '' }}>Officer Level - 10th (अधिकृत तह - १०)</option>
                                <option value="Officer Level - 8th (अधिकृत तह - ८)"  {{ old('position_level', $job->position_level) == 'Officer Level - 8th (अधिकृत तह - ८)'  ? 'selected' : '' }}>Officer Level - 8th (अधिकृत तह - ८)</option>
                                <option value="Officer Level - 7th (अधिकृत तह - ७)"  {{ old('position_level', $job->position_level) == 'Officer Level - 7th (अधिकृत तह - ७)'  ? 'selected' : '' }}>Officer Level - 7th (अधिकृत तह - ७)</option>
                                <option value="Officer Level - 6th (अधिकृत तह - ६)"  {{ old('position_level', $job->position_level) == 'Officer Level - 6th (अधिकृत तह - ६)'  ? 'selected' : '' }}>Officer Level - 6th (अधिकृत तह - ६)</option>
                            </optgroup>
                            <optgroup label="Assistant Level (सहायक तह)">
                                <option value="Officer Level - 5th (अधिकृत तह - ५)"  {{ old('position_level', $job->position_level) == 'Officer Level - 5th (अधिकृत तह - ५)'  ? 'selected' : '' }}>Officer Level - 5th (बरिष्ठ सहायक तह - ५)</option>
                                <option value="Assistant Level - 4th (सहायक तह - ४)" {{ old('position_level', $job->position_level) == 'Assistant Level - 4th (सहायक तह - ४)' ? 'selected' : '' }}>Assistant Level - 4th (सहायक तह - ४)</option>
                            </optgroup>
                            <optgroup label="Technician Level (सहयोगी)">
                                <option value="Technician Level (सहयोगी)" {{ old('position_level', $job->position_level) == 'Technician Level (सहयोगी)' ? 'selected' : '' }}>Technician (टेक्निशियन)</option>
                            </optgroup>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select form-select-lg @error('department') is-invalid @enderror"
                            ...
                            <option value="Lab"           {{ old('department', $job->department) == 'Lab'           ? 'selected' : '' }}>Technical / Lab (प्राविधिक / ल्याव)</option>
                    ...
                    <div class="mb-4">
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-check-inline border rounded p-3 w-100">
                                    <input class="form-check-input" type="radio" name="category" id="category_open"
                                        value="open" {{ old('category', $job->category) == 'open' ? 'checked' : '' }} required>
                        ...
                        <div class="inclusive-subcategory {{ old('category', $job->category) == 'inclusive' ? 'show' : '' }}" id="inclusiveSubCategory">
                            ...
                            <select class="form-select form-select-lg @error('inclusive_type') is-invalid @enderror"
                                ...
                                <option value="Pichadiyeko Chetra" {{ old('inclusive_type', $job->inclusive_type) == 'Pichadiyeko Chetra' ? 'selected' : '' }}>Pichadiyeko Chetra (पिचडिएको क्षेत्र)</option>
                            </select>
            ...
            <div class="col-lg-4">
                <div class="preview-card">
                    ...
                    <table class="preview-table">
                        <tbody>
                            <tr><th>Advertisement No.</th><td id="preview-adv-no" class="fw-semibold">{{ $job->advertisement_no }}</td></tr>
                            <tr><th>Position/Level</th><td id="preview-position" class="fw-semibold">{{ $job->position_level }}</td></tr>
                            <tr><th>Department</th><td id="preview-service" class="fw-semibold">{{ $job->department }}</td></tr>
                            ...
                            <tr id="preview-inclusive-row" style="display: {{ $job->inclusive_type ? '' : 'none' }};"><th>Inclusive Type</th><td id="preview-inclusive-type" class="fw-semibold">{{ $job->inclusive_type ?? '-' }}</td></tr>
                            <tr><th>Demand Post</th><td id="preview-posts" class="fw-semibold">{{ $job->number_of_posts }}</td></tr>
                            <tr><th>Deadline (BS)</th><td id="preview-deadline-bs" class="fw-semibold text-primary">-</td></tr>
                            <tr><th>Deadline (AD)</th><td id="preview-deadline-ad" class="fw-semibold text-secondary">{{ $job->deadline->format('M d, Y') }}</td></tr>
                        </tbody>
                    </table>
                    <div class="mt-4 p-3 bg-white rounded border">
                        <h6 class="small fw-bold text-muted mb-2"><i class="bi bi-mortarboard-fill me-1"></i>Min. Qualification</h6>
                        <p id="preview-qualification" class="small mb-0 text-muted">{{ Str::limit($job->minimum_qualification, 100) }}</p>
                    </div>
                    <div class="mt-4 p-3 rounded"
                        ...
                        <span class="status-badge {{ $job->status == 'active' ? 'bg-success text-white' : ($job->status == 'draft' ? 'bg-warning text-dark' : 'bg-danger text-white') }}">
                            {{ ucfirst($job->status) }}
                        </span>
                        <small class="text-muted d-block mt-2">Posted: {{ $job->created_at->format('M d, Y') }}</small>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body py-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <a href="{{ route('hr-administrator.jobs.index') }}" class="btn btn-outline-secondary btn-lg">
...
(function() {
    ...
    function initializeForm() {
            ...
            if (v && v !== lastBSValue && v !== 'YYYY-MM-DD' && v.length >= 10) {
                ...
                if (adVal) {
                    ...
                    if (previewDeadlineAD) {
                        ...
                        if (!isNaN(d.getTime())) previewDeadlineAD.textContent = d.toLocaleDateString('en-US', {year:'numeric',month:'short',day:'numeric'});
                    }
                }
            }
        }, 200);
        ...
        categoryRadios.forEach(r => r.addEventListener('change', toggleInclusiveSubCategory));
        ...
        const mappings = {
            'advertisement_no':     'preview-adv-no',
            'position_level':       'preview-position',
            'department':        'preview-service',
            'number_of_posts':      'preview-posts',
        ...
        Object.keys(mappings).forEach(function(id) {
            ...
            if (el && pr) {
                ...
                el.addEventListener(ev, function() {
                    ...
                    pr.textContent = v.substring(0, 100) + (v.length > 100 ? '...' : '') || '-';
                });
            }
        });
        ...
        if (catPreview) {
            categoryRadios.forEach(r => r.addEventListener('change', function() {
