...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-1">
                Notifications
                <span class="badge bg-warning text-dark" style="font-size: 0.6rem; vertical-align: middle;">HR ADMINISTRATOR</span>
            </h2>
            <p class="text-muted mb-0">View and manage your notifications</p>
        </div>
        @if($notifications->where('is_read', false)->count() > 0)
            <form method="POST" action="{{ route('hr-administrator.notifications.markAllAsRead') }}" class="d-inline">
                @csrf
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="d-flex gap-2">
                                        @if($notification->related_type === 'application' && $notification->related_id)
                                            <a href="{{ route('hr-administrator.applications.show', $notification->related_id) }}"
                                        ...
                                        @if(!$notification->is_read)
                                            <form method="POST" action="{{ route('hr-administrator.notifications.markAsRead', $notification->id) }}" class="d-inline">
                                                @csrf
                                        ...
                                        <form method="POST" action="{{ route('hr-administrator.notifications.destroy', $notification->id) }}" class="d-inline"
    ...
    @else
        ...
        <div class="card shadow-sm">
            <div class="card-body text-center py-5">
                ...
                <a href="{{ route('hr-administrator.dashboard') }}" class="btn btn-primary mt-3">
