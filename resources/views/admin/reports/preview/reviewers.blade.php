...
@section('title', 'Reviewers Report — Preview')
...
@php
    ...
    $downloadUrl = route('admin.reports.download.reviewers') . ($qs ? '?' . $qs : '');
@endphp

<div class="preview-header">
    <div>
        <h5>{{ __('admin.report_preview_reviewers') }}</h5>
        <div class="meta">{{ $reviewers->count() }} {{ __('admin.record_s') }} &nbsp;|&nbsp; {{ __('admin.generated_label') }} {{ now()->format('d M Y, h:i A') }}</div>
    </div>
        ...
        <table class="preview-table">
            <thead>
                <tr>
                    ...
                    <th>{{ __('admin.total_assigned') }}</th>
                    <th>{{ __('admin.reviewed') }}</th>
            ...
            <tbody>
                @forelse($reviewers as $i => $r)
                    <tr>
                        ...
                        <td>{{ __('admin.' . ($r->status ?? 'active')) }}</td>
                        <td>{{ $r->application_forms_count ?? 0 }}</td>
                        <td>{{ $r->reviewed_count ?? 0 }}</td>
                        <td>{{ $r->pending_count ?? 0 }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="empty-state">No reviewers found for the selected filters.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
